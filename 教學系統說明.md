...
## 🧰 新增：ESP32 × MQTT 工作坊（3~5 小時）

如果你想在單堂課內完成「ESP32 到手機的端到端 MQTT 實作」，請參考：

- 教材入口：`tutorials/esp32-mqtt-workshop/README.md`
- 安裝與環境：`tutorials/esp32-mqtt-workshop/install.md`
- 投影片（Marp）：`tutorials/esp32-mqtt-workshop/slides/iot-mqtt-workshop.marp.md`
- ESP32 範例：`tutorials/esp32-mqtt-workshop/arduino/`
- 本地 Broker：`tutorials/esp32-mqtt-workshop/server/`
- 手機網頁：`tutorials/esp32-mqtt-workshop/web/`

此工作坊包含 Mosquitto/Aedes 兩種 Broker 方案、四個 Arduino 範例、Topic 設計指南、接線/架構圖與 PowerShell 測試腳本，3~5 小時即可完整跑通。
