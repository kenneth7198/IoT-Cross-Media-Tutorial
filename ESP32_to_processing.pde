...
// Device hostname for network identification
const char* DEVICE_HOSTNAME = "ESP32-IoTDevice";  // <--- 設備名稱，可在區網內識別

// Static IP Configuration for ESP32
IPAddress local_IP(192, 168, 100, 200);      // <--- ESP32 的固定 IP
IPAddress gateway(192, 168, 100, 1);         // <--- 路由器閘道 IP
IPAddress subnet(255, 255, 255, 0);          // <--- 子網路遮罩
IPAddress primaryDNS(8, 8, 8, 8);            // (可選) Google DNS
IPAddress secondaryDNS(8, 8, 4, 4);          // (可選) Google DNS

// Set this to the IP address of the computer running Processing
const char* SERVER_IP   = "192.168.100.100";   // <--- 改成你的電腦 IP
